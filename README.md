...
校园网、企业内网、公共 WiFi 等通常开启了 **AP/客户端隔离**，同一 WiFi 下的设备之间无法互相访问，即使两台设备紧挨着也不行。这是路由器的安全策略，与信号强弱无关。

如果你发现另一台设备无法访问内网地址，大概率是网络隔离导致的。此时请改用 **模式二（快速隧道）**。
